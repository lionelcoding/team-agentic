...
import { Bot, Search, Grid3X3, GitBranch } from "lucide-react"
...
import { AGENTS, POLE_COLORS } from "@/lib/agents-data"
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-1.5 bg-slate-900 border border-slate-800 rounded-lg p-1">
          <Button size="sm" variant="ghost"
            className={cn("h-7 gap-1.5 text-xs px-3", view === "grid" ? "bg-slate-700 text-slate-100" : "text-slate-500 hover:text-slate-300")}
            onClick={() => setView("grid")}>
            <Grid3X3 size={13} /> Grille
          </Button>
          <Button size="sm" variant="ghost"
            className={cn("h-7 gap-1.5 text-xs px-3", view === "org" ? "bg-slate-700 text-slate-100" : "text-slate-500 hover:text-slate-300")}
            onClick={() => setView("org")}>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="relative w-52">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Chercher un agent..." className="h-8 pl-8 text-xs bg-slate-900 border-slate-700 text-slate-300 placeholder:text-slate-600" />
        </div>
        ...
        <div className="flex items-center gap-1 flex-wrap">
          {POLES.map(p => {
            ...
            return (
              <button key={p} onClick={() => setPoleFilter(p as Pole | "ALL")}
                className={cn(
                  "text-xs px-2.5 py-1 rounded-full border font-medium transition-colors",
                  active
                    ? color ? cn(color.bg, color.text, color.border) : "bg-slate-700 text-slate-200 border-slate-600"
